test_that("Throws error if particles dim doesn't match weight", {
  ...
  expect_error(
    .resample_multinomial_old(particles, weights),
    "Number of particles must match the length of weights"
  )
  expect_error(
    .resample_stratified_old(particles, weights),
    "Number of particles must match the length of weights"
  )
  expect_error(
    .resample_systematic_old(particles, weights),
    "Number of particles must match the length of weights"
  )
...
test_that("Multinomial resampling produces valid output", {
  ...
  resampled <- .resample_multinomial_old(particles, weights)
...
test_that("Stratified resampling produces valid output", {
  ...
  resampled <- .resample_stratified_old(particles, weights)
...
test_that("Systematic resampling produces valid output", {
  ...
  resampled <- .resample_systematic_old(particles, weights)
...
test_that("Resampling handles uniform weights correctly", {
  ...
  resampled_multinomial <- .resample_multinomial_old(particles, weights)
  resampled_stratified <- .resample_stratified_old(particles, weights)
  resampled_systematic <- .resample_systematic_old(particles, weights)
